# create a systemd service to automatically mount //deepthought/common at boot
{ inputs, config, pkgs, ... }:
let
  ...
  fileSystems."/mnt/common" = {
    ...
    options =
      let
        ...
        separate_options = "x-systemd.automount,noauto,x-systemd.idle-timeout=60,x-systemd.device-timeout=5s,x-systemd.mount-timeout=5s,uid=${builtins.toString config.users.users.${config.hostSpec.username}.uid}";
      in
